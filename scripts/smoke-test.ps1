...
Write-Host "Cursivis Nova backend smoke test"
...
try {
    ...
    if ($statusCode -eq 429) {
        Write-Warning "Analyze is rate-limited (429). Backend is healthy but Nova quota is temporarily exhausted."
        if ($bodyText) { Write-Host "Details: $bodyText" }
        Write-Host "Suggestion: wait and retry, or check your Bedrock service quotas in the AWS console."
        exit 2
    }
