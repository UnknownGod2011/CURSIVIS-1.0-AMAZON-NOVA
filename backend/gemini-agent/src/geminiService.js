    ...
    try {
      const request =
        selectionKind === "image" && imageBase64
                ...
                contents: [
                  {
                    ...
                    parts: [
                      {
                        text: buildTextImageIntentRouterPrompt({
                          text,
                          mode,
                          actionHint,
                          voiceCommand
          ...
          : {
              model,
              contents: buildIntentRouterPrompt({
                text,
                mode,
                actionHint,
                voiceCommand
      ...
      const { response, model: resolvedModel } = await withGoogleGenAiClient(
      ...
      return {
        ...fallbackIntentDecision({ selectionKind, text }),
        latencyMs: Date.now() - startedAt,
        model
      };
    }
