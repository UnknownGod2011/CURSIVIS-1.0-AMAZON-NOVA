...
export function inferUsefulEmailAction(text) {
  ...
  if (looksLikeMailboxEmail(text) || looksLikePolishedEmail(text)) {
    return "draft_reply";
  }
...
export function normalizeIntentDecision(routerOutput, selectionText = "") {
  ...
  if (
    ...
    normalizedType = fallbackType;
    if (!hasRouterAction || bestAction === "answer_question") {
      bestAction = fallbackActionForType(normalizedType);
    }
  }
  ...
  if (
    ...
    (isMetaToolAction(bestAction) || GENERIC_TEXT_ACTIONS.has(bestAction))
  ) {
    normalizedType = fallbackType;
    bestAction = fallbackActionForType(normalizedType);
  }
